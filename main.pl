...
all_lists(Num, Elements, Result) :- ground(Elements), !, length(EmptyList, Num), populate_list(EmptyList, Elements, Result).
all_lists(Num, Elements, Result) :- length(EmptyList, Num), populate_list(EmptyList, [add, subtract, multiply, divide], Result).

combo(Operators, Numbers, Result) :- length(Numbers, L), Len is L - 1, all_lists(Len, Operators, Operators), apply_lr(Operators, Numbers, Result).
